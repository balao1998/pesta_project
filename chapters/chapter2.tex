...
\item \textbf{DEEclass.cls} -- este ficheiro define a classe de documento \verb|DEEclass|. É veementemente desaconselhado a sua modificação.
...
\begin{itemize}
\item A linha \verb|\include{front/1_dedicatory}| refere-se à secção ``Dedicatória''. Se pretende escrever uma dedicatória edite o ficheiro \path{/front/1_dedicatory.tex}. Caso contrário basta comentar a linha para que o conteúdo do ficheiro não seja considerado.
\item A linha \verb|\include{front/2_acknowledgements}| refere-se à secção ``Agradecimentos'' e para a editar aceda ao ficheiro \path{/front/2_acknowledgements.tex}.
\item A linha \verb|\include{front/3_abstract}| refere-se às secções ``Resumo'' e ``Abstract'', e o seu conteúdo é alterado no ficheiro \path{/front/3_abstract.tex}. Note que terá sempre que escrever esta secção em dois idiomas, leia atentamente as instruções contidas no ficheiro. É nesta secção onde também são introduzidas as palavras-chave associadas ao seu trabalho.
\item A linha \verb|\include{front/4_frontmatterlists}| refere-se às secções ``Lista de Figuras'', ``Lista de Tabelas'', ``Listagens'', ``Lista de Algoritmos'', ``Glossário'', ``Lista de Acrónimos'' e ``Lista de Símbolos''. Deverá editar o conteúdo do ficheiro \path{/front/4_frontmatterlists.tex} para selecionar (comentando ou não as respetivas linhas) que secções deverão fazer parte do seu documento.
\end{itemize}

Qualquer uma das linhas pode ser comentada caso não queira usar a respetiva secção, a \verb|DEEclass| trata de todos os aspetos de paginação e formatação do documento final. Para adicionar conteúdos às listas de termos, acrónimos e símbolos, localize a secção ``USER DEFINED LISTS'' (linha 45).

\begin{itemize}
\item A linha \verb|\input{front/0_glossary}| refere-se às secções ``Glossário''. Edite o ficheiro \path{/front/0_glossary.tex} para elaborar o dicionário do seu relatório.
\item A linha \verb|\input{front/0_acronyms}| refere-se à secção ``Lista de Acrónimos''. Edite o ficheiro \path{/front/5_listofacronyms.tex} para elaborar a sua lista de acrónimos.
\item No mesmo sentido, a linha \verb|\input{front/0_symbols}| refere-se à secção ``Lista de Símbolos'' associada ao ficheiro \path{/front/6_listofsymbols.tex}.
\end{itemize}
...
Irá encontrar na documentação deste \textit{package} (\url{ctan.org/pkg/algorithmicx}) a lista completa de comandos disponíveis para criar os mais comuns ciclos e estruturas de controlo de fluxo lógico. É também possível fazer referência a linhas específicas do algoritmo, e.g., ver código da linha~\ref{alg:line:l3}.
